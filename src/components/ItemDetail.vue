<template>
  ...
  <div class="m-2">
    <div class="flex">
      <router-link to="/items">
        <div>
          <button class="border-2 font-semibold border-gray-500 px-2 py-1 rounded-lg">Back</button>
        </div>
      </router-link>
      <div class="ml-2">
         <button @click="handleSave" class="border-2 font-semibold bg-blue-500 text-white px-2 py-1 rounded-lg">Save</button>
      </div>
      <!-- <div>{{ state.item.id }}</div> -->
    </div>
    <div>
      <div class="text-sm font-semibold">Item Name</div>
      <input @input="handleChangeItemName" :value="state.item.name" class="border font-semibold w-full p-2 rounded-lg" placeholder="Item name..." />
    </div>
    <div>
      <div class="text-sm font-semibold">Description</div>
      <input @input="handleChangeItemDescription" :value="state.item.description" class="border font-semibold w-full p-2 rounded-lg" placeholder="Item Description..." />
    </div>
    <div>
    ...
    <div v-if="isNew">
      <div class="flex my-1">
        ...
        <input @click="handleToggleWithInitialStock" :checked="state.withInitialStock" type="checkbox" class="border-4 ml-2" />
      </div>
      <div v-if="state.withInitialStock" class="flex items-center">
        <div>
          <input type="number" :value="state.qty" @input="handleInputInitialStock" class="border font-semibold w-full p-2 rounded-lg" placeholder="Initial Stock..." />
        </div>
        <div class="flex flex-col items-center m-1">
          <div class="text-center">For Project?</div>
          <select class="appearance-none p-2">
            <option v-for="project in state.projects" :key="project.id">
              {{ project.name }}  
            </option>
          </select>
        </div>
  ...
  <div class="m-2">
    ...
    <div v-if="!isNew" class="my-2">
      <button @click="handleDeleteItem" class="bg-red-700 text-white font-semibold p-2 rounded-lg">
        Delete
      </button>
    </div> 
  </div>
...
import { initialItem } from '@/modelinitials'
  ...
  setup() {
    ...
    const fetchItem = async () => {
      try {
        ...
        const response = await fetch(`${process.env.VUE_APP_BASE_URL}/items/${itemId}`, {
          headers: { 
    ...
    const handleChangeItemPrice = (e: any) => {
      state.item.price = isNaN(parseInt(e.target.value)) ? 0 : parseInt(e.target.value)
    }

    const handleChangeItemManufacturingPrice = (e: any) => {
      state.item.manufacturingPrice = isNaN(parseInt(e.target.value)) ? 0 : parseInt(e.target.value)
    }
    ...
    const handleSave = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.VUE_APP_BASE_URL}/itemsave`, {
          method: 'POST', 
          headers: {
            'authorization': store.apiKey
          }, 
    ...
    const handleDeleteItem = () => {
      const confirmation = confirm(`Are you sure you want to delete item: ${state.item.name}?`)
    }
